...
const CONTENT_TYPE_OPTIONS: { value: MarketingContentType; label: string }[] = [
...
/*
 * Grade começando na segunda-feira, igual ao calendário de tarefas.
 */
...
function MarketingCalendar() {
  ...
  /*
   * Incrementar reloadToken força uma nova busca do mesmo mês
   * (botão de atualizar e depois de criar uma postagem).
   */
  ...
  const [form, setForm] = useState<CreateMarketingPostRequest>(() =>
  ...
  /* =======================================================
     ...
     Nada de setState síncrono dentro do efeito: o estado só
     muda quando a requisição termina. O "carregando" é derivado
     da diferença entre o que foi pedido e o que chegou.
  ======================================================= */
  ...
  useEffect(() => {
    ...
    getMarketingPosts(startDate, endDate)
      .then((data) => {
        if (!cancelled) {
          setResult({ key: requestKey, posts: data, error: "" });
        }
      })
      .catch((err) => {
        console.error(err);
        ...
        if (!cancelled) {
          setResult({
            key: requestKey,
            posts: [],
            error: "Não foi possível carregar as postagens de marketing.",
          });
        }
      });
  ...
  /* =======================================================
     MODAIS: ESC + TRAVA DE SCROLL
  ======================================================= */

  const anyModalOpen = isCreateModalOpen || selectedPost !== null;
  ...
  useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      setIsCreateModalOpen((open) => (isCreating ? open : false));
    }
  ...
  const statusCounts = useMemo(() => {
    const counts: Record<MarketingPostStatus, number> = {
      planejado: 0,
      em_producao: 0,
      publicado: 0,
      cancelado: 0,
  ...
  function openCreateModal(dateKey: string = formatDateKey(new Date())) {
    setFormError("");
  ...
  function updateForm<K extends keyof CreateMarketingPostRequest>(
    field: K,
  ...
  async function handleCreatePost(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!form.title.trim()) {
      setFormError("Informe um título para a postagem.");
      return;
    }

    if (!form.scheduled_date) {
      setFormError("Informe a data da postagem.");
      return;
    }
    ...
    try {
      ...
      const createdDate = new Date(`${form.scheduled_date}T00:00:00`);
      ...
      console.error(err);
      setFormError("Não foi possível criar a postagem. Tente novamente.");
    } finally {
      setIsCreating(false);
    }
  }
          ...
          <div className="mx-auto max-w-[1380px] px-5 py-7 sm:px-8 lg:px-10 xl:px-12">
                ...
                <p className="mt-3 text-[15px] text-slate-500">
                  {loading
                    ...
                    : `${posts.length} ${
                      ...
                      } em ${formatMonthTitle(currentMonth).toLowerCase()}`}
                </p>
              </div>
            ...
            <section className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
              {STATUS_OPTIONS.map((status) => (
                <div key={status.value} className={`${cardClass} p-5`}>
                  ...
                  <p className="mt-3 text-[2rem] font-semibold leading-none tracking-[-0.03em] text-slate-900">
                    {loading ? "–" : statusCounts[status.value]}
                  </p>
                </div>
              ))}
            </section>
            ...
            <section className={`mt-5 overflow-hidden ${cardClass}`}>
              {/* CONTROLS */}
                            ...
                            {dayPosts.slice(0, 3).map((post) => (
                              <button
                                ...
                                className={`block w-full rounded-lg px-2 py-1.5 text-left transition duration-200 hover:brightness-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-900 ${
                                  platformConfig[post.platform].pill
                                } ${
          ...
          >
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateForm("scheduled_date", event.target.value)
                    }
                    required
                    className={fieldClass}
                  />
                ...
                <div>
                  <label className={labelClass} htmlFor="post-time">
                    Horário
                  </label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateForm("scheduled_time", event.target.value)
                    }
                    className={fieldClass}
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {PLATFORM_OPTIONS.map((option) => (
                    <button
                      key={option.value}
                      type="button"
                      onClick={() => updateForm("platform", option.value)}
                      aria-pressed={form.platform === option.value}
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {CONTENT_TYPE_OPTIONS.map((option) => (
                    <button
                      key={option.value}
                      type="button"
                      onClick={() => updateForm("content_type", option.value)}
                      aria-pressed={form.content_type === option.value}
                      className={`${optionBaseClass} ${
                        form.content_type === option.value
            ...
            <div className="flex shrink-0 flex-col-reverse gap-3 border-t border-slate-100 bg-white px-6 py-4 sm:flex-row sm:justify-end sm:px-8">
              ...
              >
                ...
                {isCreating ? "Criando..." : "Criar postagem"}
              </button>
            </div>
          </div>
        </div>
                ...
                <div className="mb-2.5 flex flex-wrap items-center gap-2">
                  <span
                    className={`rounded-full px-2.5 py-1 text-xs font-medium ${
                      platformConfig[selectedPost.platform].pill
                    }`}
                  >
                    {platformConfig[selectedPost.platform].label}
                  </span>

                  <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600">
                    {contentTypeLabels[selectedPost.content_type]}
                  </span>
                </div>
            ...
            <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-6 sm:px-8">
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-xl bg-slate-50/80 px-4 py-3">
                  ...
                  <p className="mt-1 text-sm font-medium text-slate-800">
                    {formatLongDate(selectedPost.scheduled_date)}
                  </p>
                </div>
              ...
              <div>
                ...
                <span
                  className={`mt-2 inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-sm font-medium ${
                    statusConfig[selectedPost.status].active
                  }`}
                >
                  <span
                    className={`h-1.5 w-1.5 rounded-full ${
                      statusConfig[selectedPost.status].dot
                    }`}
                  />
              ...
              <div>
                ...
                <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-600">
                  {selectedPost.description || "Nenhuma descrição adicionada."}
                </p>
              </div>
